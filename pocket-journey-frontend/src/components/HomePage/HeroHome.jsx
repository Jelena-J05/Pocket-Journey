        ...
        try {
            ...
            if (activeTab === "FLIGHTS") {
                data = data.filter((flight) => {
                    return (
                        flight.from.includes(searchData.from) &&
                        flight.to.includes(searchData.to) &&
                        (!searchData.departureDate ||
                            flight.departureDate.startsWith(
                                searchData.departureDate
                            )) &&
                        (searchData.isOneWay ||
                            flight.returnDate.startsWith(searchData.returnDate))
                    )
                })
            }
            if (activeTab === "TRAINS") {
                data = data.filter((train) => {
                    return (
                        train.from.includes(searchData.from) &&
                        train.to.includes(searchData.to) &&
                        (!searchData.departureDate ||
                            train.departureDate.startsWith(
                                searchData.departureDate
                            )) &&
                        (searchData.isOneWay ||
                            train.returnDate.startsWith(searchData.returnDate))
                    )
                })
            }
                                ...
                                <Form onSubmit={handleSearchClick}>
                                    ...
                                    <Row>
                                        <Col xs={12}>
                                            <Button
                                                type="submit"
                                                className="SearchButton button-style fw-bold d-flex align-items-center"
                                            >
                                                SEARCH
                                                <FontAwesomeIcon
                                                    icon={faSearchengin}
